...
<?php endif; ?><?php echo e(PHP_EOL); ?>
        ...
        $model->saveOrFail();
<?php if(count($relations)): ?>
<?php if(isset($relations['hasMany']) && count($relations['hasMany'])): ?><?php echo e(PHP_EOL); ?>

<?php $__currentLoopData = $relations["hasMany"]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $hasManyRelations): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
<?php $__currentLoopData = $hasManyRelations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $relation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        /*-----Auot Gen Create-<?php echo e(Str::snake($key)); ?>-Start-----*/
        if (isset($data-><?php echo e(Str::snake($key)); ?>)) {
            foreach ($data-><?php echo e(Str::snake($key)); ?> as $child) {
                $model-><?php echo e(Str::plural($key)); ?>()
                ->create((array) $child);
            }

        }
        /*-----Auot Gen Create-<?php echo e(Str::snake($key)); ?>-End-----*/
...
<?php $__currentLoopData = $relations["hasMany"]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $hasManyRelations): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php echo e(PHP_EOL); ?>

<?php $__currentLoopData = $hasManyRelations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $relation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        /*-----Auot Gen Update-<?php echo e(Str::snake($key)); ?>-Start-----*/
        if (isset($data-><?php echo e(Str::snake($key)); ?>)) {
            foreach ($data-><?php echo e(Str::snake($key)); ?> as $child) {
                if(isset($child->id)){
                    $id = explode("@", OpensslHelper::decrypt($child->id));
                    $child->id = $id[0];
                    $this->model-><?php echo e(Str::plural($key)); ?>()->where('id',$child->id)
                    ->update((array) $child);
                }else{
                    $this->model-><?php echo e(Str::plural($key)); ?>()
                    ->create((array) $child);
                }
            }

        }
        /*-----Auot Gen Update-<?php echo e(Str::snake($key)); ?>-End-----*/
...
<?php endif; ?>
    ...
    {
<?php if(isset($childColumns) && count($childColumns)): ?>
<?php $__currentLoopData = $childColumns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $columns): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
		$this->model-><?php echo e(Str::plural(Str::camel($key))); ?>()->delete();
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
<?php endif; ?>
...
<?php
    ...
    try {
        $htmlType = strtolower(json_decode($col['comment'])->type);
    } catch (\Throwable $th) {
        $htmlType = null;
    };
?>
...
<?php elseif(strpos(strtolower($col), "file") !== false): ?>
<?php $len++; ?>
			Column::make("<?php echo e($col); ?>")->className('min-tablet')->orderable(false)->searchable(false)->type("<?php echo e($type); ?>"),
<?php elseif($col==='created_at'|| $col==='updated_at'): ?>
<?php $last[] = 'Column::make("'.$col.'")->className("min-tv")->type("'.$type.'")'; ?>
<?php elseif($col!='lang' && $htmlType!='html'): ?>
<?php $len++; ?>
...
<?php endif; ?>
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
<?php $last[] = "Column::make('actions')->className('all text-right')->orderable(false)->searchable(false)->type('html')"; ?>
        ];
<?php $exheader[] = '["name" => "'.$repoBaseName.'", "len" => '.$len.']' ?>


        $child_columns = [
<?php $__currentLoopData = $childColumns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $columns): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
<?php if(!isset($childrenOptions[$key]) || (isset($childrenOptions[$key]) && !in_array('multi', $childrenOptions[$key]))): ?>
<?php $len = 0; ?>
...
?>
<?php    $childrenTableName = Str::singular(Str::snake($key)); ?>
<?php if($col['name']==='name'||$col['name']==='title'): ?>
<?php $len++; ?>
            Column::make("<?php echo e($childrenTableName); ?>.<?php echo e($col['name']); ?>")->title('<?php echo e(Str::studly($col['name'])); ?>')->className('all')->type("<?php echo e($type); ?>"),
<?php elseif(strpos(strtolower($col['name']), "file") !== false): ?>
<?php $len++; ?>
            Column::make("<?php echo e($childrenTableName); ?>.<?php echo e($col['name']); ?>")->title('<?php echo e(Str::studly($col['name'])); ?>')->className('min-tablet')->orderable(false)->searchable(false)->type("<?php echo e($type); ?>"),
<?php elseif(!$col['name']!='created_at' && $col['name']!='updated_at' && $col['name']!='id' && $col['name']!='lang' && $htmlType!='html'): ?>
<?php $len++; ?>
<?php if($col['type'] == 'enum'): ?>
            Column::make("<?php echo e($childrenTableName); ?>.<?php echo e($col['name']); ?>")->title('<?php echo e(Str::studly($col['name'])); ?>')->className('min-tablet')->type("ENUM")->content(json_encode(['options' => [<?php echo $col['options']; ?>]])),
<?php else: ?>
            Column::make("<?php echo e($childrenTableName); ?>.<?php echo e($col['name']); ?>")->title('<?php echo e(Str::studly($col['name'])); ?>')->className('min-tablet')->type("<?php echo e($type); ?>"),
<?php endif; ?>
<?php endif; ?>
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
<?php $exheader[] = '["name" => "'.Str::title(str_replace("_"," ",Str::plural($key))).'", "len" => '.$len.']' ?>
<?php endif; ?>
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ];

        $exheader = [
<?php if(count($exheader) > 1): ?>
<?php $__currentLoopData = $exheader; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php echo $val; ?>,
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
<?php endif; ?>
        ];
...
<?php if(isset($childColumns) && count($childColumns)): ?>
        $query = $query->with([
<?php $__currentLoopData = $childColumns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $columns): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            '<?php echo e(Str::singular(Str::studly(Str::snake($key)))); ?>',
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ]);
<?php endif; ?>
...
<?php $childrenTableName = Str::singular(Str::snake($key)); ?>
        $childColumns = [
<?php $__currentLoopData = $columns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $col): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            '<?php echo e($childrenTableName); ?>.<?php echo e($col['name']); ?>',
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ];
...
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
<?php $__currentLoopData = $childColumns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $columns): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
<?php if( in_array(Str::snake($key),$children_tableName)): ?>
<?php    $childrenTableName = Str::snake($key); ?>
<?php else: ?>
<?php    $childrenTableName = Str::singular(Str::snake($key)); ?>
<?php endif; ?>
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        return DataTables::of($query)->editColumn('actions',function (<?php echo e($modelBaseName); ?> $model) {
...
<?php /**PATH /var/www/online_form/vendor/lesliew/laravel-jetin-generator/src/../resources/views/templates/withChildren/repository.blade.php ENDPATH**/ ?>