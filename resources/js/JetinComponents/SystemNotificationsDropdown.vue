<template>
  ...
  <jet-dropdown align="right" content-classes="p-0 bg-white w-full">
      ...
      <slot name="trigger">
        ...
        >
          <span
            ...
            class="absolute z-20 bg-danger p-0.5 rounded -bottom-0.5 -right-1 px-2 font-black text-sm text-white"
            >{{ notifications.filter((n) => !n.read_at).length }}</span
          >
          <svg
    ...
    <template #content>
      <div class="py-2 mb-2" v-if="notifications.filter((n) => !n.read_at).length">
        <template v-for="(noti, index) of notifications" :key="index">
            ...
            >
                ...
                <span class="flex flex-wrap mx-2 text-sm text-left text-gray-600">
      ...
      <div v-else class="py-5 mb-2 text-center">{{__("You have no unread notifications")}}</div>
      <inertia-button
        :href="route('dashboard')"
        class="block py-2 font-bold text-center text-white bg-green-500 rounded-t-none"
        >{{__("See all notifications")}}</inertia-button
      >
    </template>
    ...
    >
      ...
      <template #footer class="inline-block text-right gap-x-2">
        <inertia-button @click.native.prevent="closeNotification" class="bg-gray-300">{{
          __("Close")
        }}</inertia-button>
        <a
          target="_blank"
...
export default defineComponent({
  ...
  mounted() {
    ...
    document.onreadystatechange = () => {
        ...
        // window.Echo.private('notify_users.' + this.$page.props.auth.user.id)
        // .notification((notification) => {
        //     console.log(notification);
        // });
