<template>
    <jetin-layout :title="__('Edit') + '-' + __('Role')">
        <template #navbar-button>
            <div class="flex flex-wrap items-center justify-between w-full px-4">
                <inertia-link :href="route('admin.roles.index')" class="text-xl font-black text-zinc-400"><i class="fas fa-arrow-left"></i> {{__('Back')}} | {{__('Edit')}}
                    {{__("Role")}} - {{model.name}}</inertia-link>
            </div>
        </template>
