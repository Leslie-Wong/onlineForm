<template>
    <jetin-layout :title="__('Form')">
            ...
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md w-full">
                ...
                <div v-if="showModal && currentModel">
                    <jetin-modal
                        ...
                        <template #title>{{__('Show')}} {{__("Form")}} #{{currentModel.id}}</template>
                        <show-forms-form :model="currentModel"></show-forms-form>
                        <template #footer>
        ...
        {{__('You are not authorized to view a list of')}} {{__("Forms")}}
        </div>
    </jetin-layout>
</template>
...
<script>
    ...
    import ShowFormsForm from "@/Pages/Forms/ShowForm.vue";
    ...
    export default defineComponent({
        ...
        computed: {
            ajaxUrl() {
                const url = new URL(this.route('api.forms.dt'));
        ...
        methods: {
            showModel(model) {
                axios.get(route('api.forms.show',model)).then(res => {
                    this.currentModel = res.data.payload;
                    this.showModal = true;
                })
                // this.$inertia.visit(this.route('admin.forms.show',model.id));
            },
            editModel(model) {
                this.$inertia.visit(this.route('admin.forms.edit',model.id));
            },
            ...
            async deleteModel() {
                ...
                if (this.currentModel) {
                    this.$inertia.delete(route('admin.forms.destroy', vm.currentModel),{
                        onFinish: res => {
            ...
            async toggleActivation(enabled,model) {
                const vm = this;
                console.log(enabled);
                axios.put(route(`api.forms.update`,model.id),{
                    enabled: enabled
                }).then(res => {
