<template>
    <jetin-layout :title="__('Language')">
            ...
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md w-full">
                ...
                <div v-if="showModal && currentModel">
                    <jetin-modal
                        ...
                        <template #title>{{__('Show')}} {{__("Language")}} - {{currentModel.name}}</template>
                        <show-languages-form :model="currentModel"></show-languages-form>
                        <template #footer>
        ...
        {{__('You are not authorized to view a list of')}} {{__("Languages")}}
        </div>
    </jetin-layout>
</template>
...
<script>
    ...
    import ShowLanguagesForm from "@/Pages/Languages/ShowForm.vue";
    ...
    export default defineComponent({
        ...
        computed: {
            ajaxUrl() {
                const url = new URL(this.route('api.languages.dt'));
        ...
        methods: {
            showModel(model) {
                axios.get(route('api.languages.show',model)).then(res => {
                    this.currentModel = res.data.payload;
                    this.showModal = true;
                })
                // this.$inertia.visit(this.route('admin.languages.show',model.id));
            },
            editModel(model) {
                this.$inertia.visit(this.route('admin.languages.edit',model.id));
            },
            ...
            async deleteModel() {
                ...
                if (this.currentModel) {
                    this.$inertia.delete(route('admin.languages.destroy', vm.currentModel),{
                        onFinish: res => {
            ...
            async toggleActivation(enabled,model) {
                const vm = this;
                console.log(enabled);
                axios.put(route(`api.languages.update`,model.id),{
                    enabled: enabled
                }).then(res => {
