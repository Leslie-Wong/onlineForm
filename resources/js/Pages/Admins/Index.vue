<template>
    <jetin-layout :title="__('Admin')">
            ...
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md w-full">
                ...
                <div v-if="showModal && currentModel">
                    <jetin-modal
                        ...
                        <template #title>{{__('Show')}} {{__("Admin")}}  - {{currentModel.name}}</template>
                        <show-admins-form :model="currentModel"></show-admins-form>
                        <template #footer>
        ...
        <div v-else class="p-4 font-bold text-red-500 bg-red-100 rounded-md shadow-md ">
            {{__('You are not authorized to view a list of')}} {{__("Admin")}}
        </div>
    </jetin-layout>
</template>
...
<script>
    ...
    import ShowAdminsForm from "@/Pages/Admins/ShowForm.vue";
    export default {
        name: "Index",
        components: {
            DtComponent,
        ...
        computed: {
            ajaxUrl() {
                const url = new URL(this.route('api.admins.dt'));
        ...
        methods: {
            showModel(model) {
                axios.get(route('api.admins.show',model)).then(res => {
                    this.currentModel = res.data.payload;
                    this.showModal = true;
                })
                // this.$inertia.visit(this.route('admin.admins.show',model.id));
            },
            editModel(model) {
                this.$inertia.visit(this.route('admin.admins.edit',model.id));
            },
            ...
            async deleteModel() {
                ...
                if (this.currentModel) {
                    this.$inertia.delete(route('admin.admins.destroy', vm.currentModel),{
                        onFinish: res => {
            ...
            async toggleActivation(enabled,model) {
                const vm = this;
                console.log(enabled);
                axios.put(route(`api.admins.update`,model.id),{
                    enabled: enabled
                }).then(res => {
