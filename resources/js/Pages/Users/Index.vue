<template>
    <jetin-layout :title="__('User')">
            ...
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md w-full">
                ...
                <div v-if="showModal && currentModel">
                    <jetin-modal
                        ...
                        <template #title>{{__('Show')}} {{__("User")}}  - {{currentModel.name}}</template>
        ...
        <div v-else class="p-4 font-bold text-red-500 bg-red-100 rounded-md shadow-md ">
            {{__('You are not authorized to view a list of')}} {{__("Users")}}
        </div>
    </jetin-layout>
</template>
